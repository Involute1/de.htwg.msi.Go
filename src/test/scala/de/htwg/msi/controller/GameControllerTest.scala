  ...
  "GameController" should {
    ...
    "print correct empty board" in {
      gameController.printGameBoard(List(
        ...
        List(Field(0, 3), Field(1, 3), Field(2, 3), Field(3, 3)))).trim.replaceAll("\n", "").replaceAll("\r", "") should be(
        """|0   A  B  C  D 
           |    -  -  -  - 
           |A | O  O  O  O
           |B | O  O  O  O
           |C | O  O  O  O
           |D | O  O  O  O""".stripMargin.trim.replaceAll("\n", "").replaceAll("\r", ""))
    }
    "print correct full board" in {
      gameController.printGameBoard(List(
        ...
        List(Field(0, 3, Some(WHITE)), Field(1, 3, Some(BLACK)), Field(2, 3, Some(WHITE)), Field(3, 3, Some(WHITE))))).trim.replaceAll("\n", "").replaceAll("\r", "") should be(
        """|0   A  B  C  D 
           |    -  -  -  - 
           |A | w  b  w  w
           |B | b  w  w  b
