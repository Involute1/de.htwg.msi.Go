      ...
      "return the parsed sgf file as Success on correct sgf file" in {
        externalDSLParser.parseDSL(
          """(;GM[1]
            ...
            |OT[10x20 byo-yomi]
            |;B[dp];W[pp];B[pd])"""
            .stripMargin) mustBe Left(SgfData(SgfGameData(19, "MIss", "petgo3"), List(Move("B", "dp"), Move("W", "pp"), Move("B", "pd"))))
      }
      "return the an error message on incorrect sgf file" in {
        externalDSLParser.parseDSL(
          """(;GM[1]
            |FF[4])"""
            .stripMargin).getOrElse("").replaceAll("[\n|\r]", "").replaceAll(" +", " ") mustBe """[1.1] failed parsing: string matching regex '[\s\S]*?(?=SZ)' expected but '(' found (;GM[1]^)"""
      }
    }
  }
