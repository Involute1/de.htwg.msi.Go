...
class PlayingStateTest extends AnyWordSpec {
  "PlayingState" should {
    ...
    "return ForfeitState after forfeit input" in {
      playingState.evaluate("forfeit").left.get mustBe a[ForfeitState]
    }
    "return error Msg for invalid move" in {
      playingState.evaluate("123") must be(Right("Input invalid try again"))
    }
    "return PlayingState after valid move" in {
      playingState.evaluate("AA").left.get mustBe a[PlayingState]
    }
    "changes ControllerState to ForfeitState" in {
      playingState.nextState(gameData) must be(ForfeitState(gameData))
    }
    /*"return available Moves for the current player as controller message" in {
      playingState.getControllerMessage() mustBe (
        """[ 0   A  B 
          |    -  - 
          |A | O O
          |B | O O ]
          |Player Player2 enter one of the following:
          |
          |AA,AB,BA,BB
